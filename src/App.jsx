...
function CargoModal({ onClose, onSave, entry }) {
  ...
  const [bonoVenta,  setBonoVenta]  = useState(entry?.bonoVenta  || "");
  const fi  = { width:"100%",boxSizing:"border-box",background:"#181b22",border:"1px solid #1e2330",borderRadius:8,padding:"9px 12px",color:"#e8ecf4",fontFamily:"inherit",fontSize:13,outline:"none" };
  const lbl = txt => <label style={{fontSize:11,color:"#7a8399",textTransform:"uppercase",letterSpacing:".08em",display:"block",marginBottom:6}}>{txt}</label>;
  const save = () => {
    if (!nombre.trim()) { alert("Ingresa el nombre del cargo"); return; }
    onSave({ nombre:nombre.trim(), tipo,
      valorVenta: tipo==="vendedor" ? (parseFloat(valorVenta)||0) : 0,
      bonoVenta:  tipo==="supervisor" ? (parseFloat(bonoVenta)||0) : 0,
    });
  };
  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,backdropFilter:"blur(4px)"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div>{lbl("Tipo")}
            <select style={fi} value={tipo} onChange={e=>setTipo(e.target.value)}>
              ...
              <option value="supervisor">Supervisor — sueldo base + bono equipo</option>
            </select>
          </div>
          {tipo==="vendedor" && (
            <div>{lbl("Valor por venta ($) — lo que paga Wom")}<input style={fi} type="number" value={valorVenta} onChange={e=>setValorVenta(e.target.value)} placeholder="70000"/></div>
          )}
          {tipo==="supervisor" && (
            <div>{lbl("Bono por venta del equipo ($)")}<input style={fi} type="number" value={bonoVenta} onChange={e=>setBonoVenta(e.target.value)} placeholder="4000"/></div>
          )}
        </div>
        <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:24}}>
          <button onClick={onClose} style={{background:"#181b22",border:"1px solid #2a3045",borderRadius:8,padding:"9px 20px",color:"#7a8399",cursor:"pointer",fontFamily:"inherit",fontSize:13}}>Cancelar</button>
...
function TrabajadorModal({ onClose, onSave, entry, cargos, trabajadores }) {
  ...
  const [month,       setMonth]       = useState(entry?.month       || MONTHS[new Date().getMonth()]);
  const [vendedoresIds, setVendedoresIds] = useState(entry?.vendedoresIds || []);

  const cargo      = cargos.find(c=>c.id===cargoId);
  const esVendedor   = cargo?.tipo === "vendedor";
  const esSupervisor = cargo?.tipo === "supervisor";

  // Vendedores disponibles del mismo mes (excluye supervisores y el propio trabajador)
  const vendedoresDisp   = trabajadores.filter(t => t.tipo === "vendedor" && t.month === month && t.id !== entry?.id);
  const vendedoresEquipo = vendedoresDisp.filter(t => vendedoresIds.includes(t.id));
  const ventasEquipo     = vendedoresEquipo.reduce((s,t)=>s+t.ventas,0);
  const sumaDifVendedores = vendedoresEquipo.reduce((s,t)=>s+t.diferencia,0);
  const bonoVenta        = cargo?.bonoVenta || 0;
  const sueldo           = parseFloat(sueldoBase)||0;
  const totalSupervisor  = esSupervisor ? sueldo + (ventasEquipo * bonoVenta) : 0;
  const costoPorVentaSup = esSupervisor && ventasEquipo>0 ? totalSupervisor / ventasEquipo : 0;
  const diferenciaSup    = esSupervisor ? sumaDifVendedores - totalSupervisor : 0;

  const pagoVentas    = esVendedor ? (parseFloat(ventas)||0) * (cargo?.valorVenta||0) : 0;
  const costoPorVenta = esVendedor && (parseFloat(ventas)||0) > 0 ? sueldo / (parseFloat(ventas)||1) : 0;
  const diferencia    = esVendedor ? pagoVentas - sueldo : 0;

  const toggleVendedor = id => setVendedoresIds(p => p.includes(id) ? p.filter(x=>x!==id) : [...p,id]);
  ...
  const save = () => {
    ...
    onSave({ nombre:nombre.trim(), cargoId, cargoNombre:cargo?.nombre||"", tipo:cargo?.tipo||"vendedor",
      ...
      pagoVentas: esVendedor ? pagoVentas : ventasEquipo * bonoVenta,
      costoPorVenta: esVendedor ? costoPorVenta : costoPorVentaSup,
      diferencia: esVendedor ? diferencia : diferenciaSup,
      totalSupervisor: esSupervisor ? totalSupervisor : 0,
      sumaDifVendedores: esSupervisor ? sumaDifVendedores : 0,
      vendedoresIds: esSupervisor ? vendedoresIds : [],
      ventasEquipo: esSupervisor ? ventasEquipo : 0,
      bonoVenta: esSupervisor ? bonoVenta : 0,
      month });
  };

  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,backdropFilter:"blur(4px)"}}>
      <div style={{background:"#111318",border:"1px solid #2a3045",borderRadius:16,padding:28,width:480,maxWidth:"95vw",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          ...
          {esSupervisor && (
            <div style={{gridColumn:"1/-1"}}>
              {lbl(`Vendedores del equipo (bono: ${fmtMoney(bonoVenta)}/venta)`)}
              {vendedoresDisp.length===0 ? (
                <div style={{background:"#181b22",border:"1px solid #1e2330",borderRadius:8,padding:"12px",fontSize:12,color:"#4a5168"}}>
                  No hay vendedores registrados para {month}. Agrégalos primero.
                </div>
              ) : (
                <div style={{background:"#181b22",border:"1px solid #1e2330",borderRadius:8,padding:10,display:"flex",flexDirection:"column",gap:6}}>
                  {vendedoresDisp.map(v=>(
                    <label key={v.id} style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",padding:"6px 8px",borderRadius:6,background:vendedoresIds.includes(v.id)?"rgba(167,139,250,.1)":"transparent"}}>
                      <input type="checkbox" checked={vendedoresIds.includes(v.id)} onChange={()=>toggleVendedor(v.id)} style={{accentColor:"#a78bfa",width:14,height:14}}/>
                      <span style={{fontSize:13,color:"#e8ecf4",flex:1}}>{v.nombre}</span>
                      <span style={{fontSize:11,color:"#7a8399"}}>{v.ventas} ventas</span>
                    </label>
                  ))}
                </div>
              )}
            ...
            {esVendedor ? (
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:10,textAlign:"center"}}>
                {[
                  ...
                  ["Costo/venta",  costoPorVenta>0?fmtMoney(Math.round(costoPorVenta)):"—", "#eab308"],
                  ["Pago Wom",     fmtMoney(pagoVentas),                                "#4f7fff"],
                  ["Diferencia",   (diferencia>=0?"+":"")+fmtMoney(diferencia),         diferencia>=0?"#22c55e":"#ef4444"],
                ].map(([l,v,c])=>(
                  <div key={l}><div style={{fontSize:10,color:"#7a8399",marginBottom:3}}>{l}</div><div style={{fontWeight:700,fontSize:14,color:c}}>{v}</div></div>
                ))}
              </div>
            ) : esSupervisor ? (
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr 1fr",gap:10,textAlign:"center"}}>
                {[
                  ["Sueldo base",    fmtMoney(sueldo),                                              "#e8ecf4"],
                  ["Bono equipo",    fmtMoney(ventasEquipo*bonoVenta),                              "#a78bfa"],
                  ["Total sup.",     fmtMoney(totalSupervisor),                                     "#f97316"],
                  ["Σ Dif. vendedores", fmtMoney(sumaDifVendedores),                               "#4f7fff"],
                  ["Diferencia",     (diferenciaSup>=0?"+":"")+fmtMoney(diferenciaSup),             diferenciaSup>=0?"#22c55e":"#ef4444"],
                ].map(([l,v,c])=>(
                  <div key={l}><div style={{fontSize:9,color:"#7a8399",marginBottom:3}}>{l}</div><div style={{fontWeight:700,fontSize:13,color:c}}>{v}</div></div>
                ))}
              </div>
            ) : (
...
export default function App() {
  ...
  return (
    <div style={{background:"#0a0b0e",minHeight:"100vh",color:"#e8ecf4",fontFamily:"system-ui,sans-serif"}}>
      ...
      {/* ── WOM GASTOS ── */}
      {view==="table" && activeDept==="Wom" && womTab==="gastos" && (
        <div style={{padding:24,display:"flex",flexDirection:"column",gap:20}}>
          <div style={{display:"flex",gap:4,borderBottom:"1px solid #1e2330",marginBottom:-8}}>
              ...
              <tbody>
                ...
                {tableData.map(e=>(
                  <tr key={e.id} style={{borderBottom:S.border}} onMouseEnter={ev=>ev.currentTarget.style.background=S.surface2} onMouseLeave={ev=>ev.currentTarget.style.background="transparent"}>
                    ...
                    <td style={{padding:"12px"}}>
                      <div>{e.desc}</div>
                      {e.isComision&&(<div style={{display:"flex",gap:8,marginTop:5,flexWrap:"wrap"}}>
                        <span style={{fontSize:11,color:"#7a8399"}}>Total: <strong style={{color:"#e8ecf4"}}>{e.qtTotal}</strong></span>
                        <span style={{fontSize:11,color:"#22c55e"}}>✅ {e.qtCorrectas}</span>
                        {e.qtIncorrectas>0&&<span style={{fontSize:11,color:"#ef4444"}}>❌ -{e.qtIncorrectas}</span>}
                        <span style={{fontSize:11,color:"#7a8399"}}>× {fmtMoney(e.valorUnitario)}</span>
                    ...
                    <td style={{padding:"12px",fontFamily:"monospace",fontWeight:600}}>{fmtMoney(e.amount)}</td>
                    <td style={{padding:"12px",fontFamily:"monospace",fontWeight:600,color:e.isComision&&e.netoComision<e.amount?"#22c55e":"#e8ecf4"}}>
                      {e.isComision?(<div>{fmtMoney(e.netoComision)}{e.qtIncorrectas>0&&<div style={{fontSize:11,color:"#ef4444",marginTop:2}}>-{fmtMoney(e.qtIncorrectas*e.valorUnitario)}</div>}</div>):fmtMoney(e.amount)}
                    </td>
                    <td style={{padding:"12px",fontSize:11,color:"#4a5168"}}>{e.by}</td>
                    <td style={{padding:"12px"}}>
                      {currentUser.role!=="reader"&&(<div style={{display:"flex",gap:4}}>
                        <button onClick={()=>setModal(e)} style={{background:S.surface2,border:S.border2,borderRadius:5,width:26,height:26,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#7a8399",fontSize:12}}>✎</button>
                        <button onClick={()=>deleteEntry(e.id)} style={{background:S.surface2,border:S.border2,borderRadius:5,width:26,height:26,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#7a8399",fontSize:12}}>✕</button>
                      </div>)}
                    </td>
      ...
      {view==="table" && activeDept==="Wom" && womTab==="remuneraciones" && (()=>{
        ...
        return (
          <div style={{padding:24}}>
            {/* Wom sub-tabs */}
            <div style={{display:"flex",gap:4,borderBottom:"1px solid #1e2330",marginBottom:16}}>
              {[["gastos","📋 Gastos"],["remuneraciones","💜 Remuneraciones"]].map(([v,lbl])=>(
                <button key={v} onClick={()=>setWomTab(v)} style={{background:"none",border:"none",borderBottom:`2px solid ${womTab===v?"#a78bfa":"transparent"}`,borderRadius:0,padding:"8px 18px",color:womTab===v?"#e8ecf4":"#7a8399",fontWeight:womTab===v?600:400,fontSize:13,cursor:"pointer",fontFamily:"inherit",marginBottom:-1}}>{lbl}</button>
              ))}
            </div>
            ...
            {remData.length>0 && remTab==="trabajadores" && (
              <div style={{background:S.surface,border:S.border,borderRadius:12,overflow:"hidden"}}>
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  <thead><tr style={{background:S.surface2,borderBottom:S.border}}>
                    {["Nombre","Cargo","Mes","Sueldo Base","Ventas","Costo/Venta","Pago Wom","Diferencia",""].map(h=>(
                    ...
                    {remData.map(t=>(
                      <tr key={t.id} style={{borderBottom:S.border}} onMouseEnter={ev=>ev.currentTarget.style.background=S.surface2} onMouseLeave={ev=>ev.currentTarget.style.background="transparent"}>
                        ...
                        <td style={{padding:"12px 14px",fontFamily:"monospace",color:"#7a8399"}}>
                          {t.tipo==="vendedor" ? `${t.ventas} × ${fmtMoney(t.valorVenta)}` : t.tipo==="supervisor" ? `${t.ventasEquipo||0} ventas equipo` : "—"}
                        </td>
                        <td style={{padding:"12px 14px",fontFamily:"monospace",color:"#eab308",fontWeight:600}}>
                          {(t.tipo==="vendedor"&&t.ventas>0)||(t.tipo==="supervisor"&&t.ventasEquipo>0) ? fmtMoney(Math.round(t.costoPorVenta||0)) : "—"}
                        </td>
                        <td style={{padding:"12px 14px",fontFamily:"monospace",color:"#4f7fff",fontWeight:600}}>
                          {t.tipo==="vendedor" ? fmtMoney(t.pagoVentas||0) : t.tipo==="supervisor" ? fmtMoney((t.ventasEquipo||0)*(t.bonoVenta||0)) : "—"}
                        </td>
                        <td style={{padding:"12px 14px",fontFamily:"monospace",fontWeight:700,color:t.tipo==="vendedor"?(t.diferencia>=0?"#22c55e":"#ef4444"):(() => {
                          if(t.tipo!=="supervisor") return "#7a8399";
                          const vEquipo = remData.filter(x=>t.vendedoresIds?.includes(x.id));
                          const sumaDif = vEquipo.reduce((s,x)=>s+x.diferencia,0);
                          const totalSup = t.sueldoBase + (t.ventasEquipo||0)*(t.bonoVenta||0);
                          return (sumaDif-totalSup)>=0?"#22c55e":"#ef4444";
                        })()}}>
                          {t.tipo==="vendedor" ? `${t.diferencia>=0?"+":""}${fmtMoney(t.diferencia)}` : t.tipo==="supervisor" ? (() => {
                            const vEquipo = remData.filter(x=>t.vendedoresIds?.includes(x.id));
                            const sumaDif = vEquipo.reduce((s,x)=>s+x.diferencia,0);
                            const totalSup = t.sueldoBase + (t.ventasEquipo||0)*(t.bonoVenta||0);
                            const dif = sumaDif - totalSup;
                            return `${dif>=0?"+":""}${fmtMoney(dif)}`;
                          })() : "—"}
                        </td>
                        <td style={{padding:"12px 14px"}}>
                          {currentUser.role==="admin" && (
              ...
              {cargos.map(c=>(
                <div key={c.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"#181b22",borderRadius:8,padding:"12px 14px"}}>
                  <div>
                    ...
                    <div style={{fontSize:11,color:"#7a8399",marginTop:2}}>{c.tipo==="vendedor"?`Vendedor · ${fmtMoney(c.valorVenta)}/venta`:`Supervisor · bono ${fmtMoney(c.bonoVenta||0)}/venta equipo`}</div>
                  </div>
                  <div style={{display:"flex",gap:4}}>
